...
import { getDeleteFavoritedTeams } from "../../actions/favorites/getDeleteFavoritedTeams";
...
const UserProfile = () => {
	...
	const favoritedLeagues = useSelector(
		(state) => state.favoritedLeagues.favorites
	);
	const favoritedTeams = useSelector(
		(state) => state.favoritedTeams.favorites
	);
	...
	const showLeagues = () => {
		if (!_.isEmpty(favoritedLeagues)) {
			return favoritedLeagues.map((el) => {
				return (
					<div
						className="user-profile__cardContainer linkWrapper fadeIn"
						key={shortid.generate()}
					>
						<Link
							className="header__link link"
							to={{
								pathname: `/allLeagues/${el.leagueId}`,
							}}
						>
						...
						<button
							className="user-profile__btn--remove btn"
							onClick={() =>
								dispatch(
									getDeleteFavoritedLeagues(
										el.leagueId
									)
								)
							}
			...
			return (
				<p className="user-profile__no-fav fadeIn">
					no favorite leagues found
				</p>
			);
		}
	};
			...
			<aside className="user-profile__sidebar">
				<nav className="user-profile__nav">
					<div className="user-profile__profile">
						<img
							className="user-profile__image fadeInFromLeft"
							src={guestAvatar}
							alt="avatar"
						/>
						...
						<button
							className="user-profile__btn btn"
